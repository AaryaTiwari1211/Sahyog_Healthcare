...
const Specialist = () => {
  ...
  const handleOnChat = async () => {
    try {
      ...
      const filteredChats = chats.filter(
        (chat) =>
          chat.users.includes(user?.user?.id) &&
          chat.users.includes(currentSpecialist.id)
      );
  ...
  const handleOnClick = () => {
    const text =
      "Hi, I want to book an appointment can you please let me know if it's possible and also the details.";
    window.open(
      `https://wa.me/7999250587?text=${encodeURIComponent(text)}`,
      "_blank"
    );
  };
    ...
    <>
      ...
      <div className="flex flex-col mt-[100px] ml-4 h-screen overflow-scroll">
        ...
        <div className="w-full">
          <img src={currentSpecialist.photo} alt="Specialist Photo" />
        </div>
        <div className="flex flex-col gap-1">
          <Typography
            color="white"
            className="mt-5 text-xl font-bold text-white font-inter"
          >
            About
          </Typography>
          <Typography
            color="gray"
            className="text-sm font-inter w-[350px] text-white"
          >
        ...
        <div className="w-full">
          <div className="flex justify-between gap-3 mt-5 mr-5 mb-28">
            <Button
              className="border-2 border-[#65ADE1]"
              variant="outlined"
              size="sm"
